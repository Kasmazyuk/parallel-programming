...
double* CreateVectorMatrix(int N){
...
double* Trivial_alghorithm(double* matrix1, double* matrix2, int N){
	double* Rez = CreateVectorMatrix(N);
	double sum;
	for(int i = 0; i < N; i++)
		for(int j = 0; j < N; j++){
			sum = 0;
...
double* Shtr_alg(double* matrix1, double* matrix2, int N, int threshold){
	double* Rez;
...
int main(int argc, char** argv){
	double *matr_A, *matr_B, *matr_Rez_Seq, *matr_Rez_Par, **A, **B, **TMP_Rez;
	...
	if (ProcRank == 0){
		...
		cout << "Par. Algoritm time = " << TimeParAlg << endl;
		char fParAlg[20] = "D:\ParAlg.txt";
			if (N < 20){
				cout << "Matr C:" << endl;
		...
		cout << "Posl. Algoritm time = " << TimeSeqAlg << endl;
		char fSeqAlg[20] = "D:\SeqAlg.txt";
		if (N < 10){
			cout << "Matr C:" << endl;
			...
			cout<< "Speed up = " << TimeSeqAlg/TimeParAlg  << endl;
		...
		for(int k = 0; k < N; k++)
			for(int l = 0; l < N; l++)
				...
				if(ceil (matr_Rez_Seq[k*N+l]) != ceil  (matr_Rez_Par[k*N+l])) 
					flag = true;


		if(flag)
			cout << "result posl. != result par." << endl;
		if(!flag)
			cout << "result posl. == result par." << endl;
		delete[] matr_Rez_Par;
		delete[] matr_Rez_Seq;
